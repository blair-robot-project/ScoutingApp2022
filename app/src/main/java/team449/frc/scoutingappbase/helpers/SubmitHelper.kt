...
fun submitMatch(match: MatchViewModel, postSumbit: Runnable, pageChanger: PageChanger?) {
    SubmissionTask(postSumbit, pageChanger).execute(MatchShadow(match))
}
...
private fun validateSoft(match: MatchShadow): ValidationError {
    ...
    if (match.high < 2*match.center) error.addError("Are you sure the robot is getting more than 2/3 of their high goal shots into the center goal? This seems unlikely at this level of competition. Ask a scouting lead for help if you are unsure.", 2)
    if (match.climbTime < 3) error.addError("Did the robot really climb in ${match.climbTime} seconds from start to finish? That's very fast.", 3)
    return error
}


class SubmissionTask(private val post: Runnable, pageChanger: PageChanger?): AsyncTask<MatchShadow, Void, Boolean>() {
